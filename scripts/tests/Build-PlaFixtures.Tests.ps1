...
Describe 'Build-PlaFixtures' {
  It 'creates exactly three .pla files' {
    New-PlaFixtures -Keywords $script:keywords -OutputDir $script:tmp
    Test-Path "$script:tmp/test_indicator.pla" | Should -BeTrue
    Test-Path "$script:tmp/test_signal.pla"    | Should -BeTrue
    Test-Path "$script:tmp/test_function.pla"  | Should -BeTrue
  }

  It 'signal file uses Strategy_Orders keywords' {
    $body = Get-Content "$script:tmp/test_signal.pla" -Raw
    $body | Should -Match '\bBuy\b'
    $body | Should -Match '\bSell\b'
  }

  It 'indicator file uses Math and Plotting keywords' {
    $body = Get-Content "$script:tmp/test_indicator.pla" -Raw
